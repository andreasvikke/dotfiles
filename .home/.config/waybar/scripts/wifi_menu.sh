...
# Prompt for password
get_password() {
  rofi -dmenu -password -theme-str "${override_password}" -p " " || pkill -x rofi
}
...
while true; do
  ...
  *"enabled"*)
    selected_option=$(echo "$options"$'\n'"$(wifi_list)" |
      rofi -dmenu -i -selected-row 1 -p " " || pkill -x rofi)
    ;;
  *"disabled"*)
    selected_option=$(echo "$option_disabled" |
      rofi -dmenu -i -theme-str "${override_disabled}" || pkill -x rofi)
    ;;
  esac
  ...
  *"Manual Entry")
    # Prompt for SSID
    manual_ssid=$(rofi -dmenu -theme-str "${override_ssid}" -p " " || pkill -x rofi)
